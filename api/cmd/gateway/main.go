...
func SetupMux(ctx context.Context, cfg proxyConfig) *http.ServeMux {
	...
	err := gw.RegisterReportServiceHandlerFromEndpoint(ctx, gwmux, cfg.backend, opts)
	if err != nil {
...
func SetupViper() *viper.Viper {
	...
	v := viper.GetViper()
	bindEnvVars(v, strings.NewReplacer(EnvVarSepChar, ViperKeySepChar), strings.ToUpper("ReportService"), ResponseHeaders)
	return v
}
